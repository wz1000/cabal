...
import System.Directory
         ...
         , makeRelativeToCurrentDirectory, getCurrentDirectory )
...
runReplOrWriteFlags
  ...
  -> PackageName
  -> IO ()
runReplOrWriteFlags verbosity ghcProg comp platform rflags replOpts bi clbi pkgName =
  case replOptionsFlagOutput rflags of
    ...
    Flag out_dir -> do
      ...
      let uid = componentUnitId clbi
          ...
          extra_opts = concat $
                      [ ["-this-package-name", prettyShow pkgName]
                      , ["-working-dir"      , src_dir]
                      ] ++
      ...
      writeFileAtomic (out_dir </> this_unit) $ BS.pack $ escapeArgs $ (extra_opts ++) $ renderGhcOptions comp platform (replOpts { ghcOptMode = NoFlag })
